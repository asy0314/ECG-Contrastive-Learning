...
parser = argparse.ArgumentParser(description='MoCo Self-supervised Pre-training')
...
parser.add_argument('--moco-k', default=15360, type=int, help='queue size; number of negative keys')
...
def main():
    ...
    if args.save == '':
        args.save = f'./outputs/seed_{args.seed}/moco/{args.arch}/pretrain'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_aupr, val_lrap, val_auroc = evaluate(labeled_train_loader, val_loader, model.encoder_q, device, 'Validation', args)
        ...
        if is_best:
            best_epoch = epoch
            best_aupr = val_aupr
            test_aupr, test_lrap, test_auroc = evaluate(labeled_train_loader, test_loader, model.encoder_q, device, 'Test', args)
...
def train(train_loader, model, optimizer, epoch, device, args):
    ...
    for i, (data_pair, target) in enumerate(train_loader):
        ...
        # MoCo model computes loss directly
