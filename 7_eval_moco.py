...
# pretrained/eval setting
parser.add_argument('--arch', required=True, type=str, metavar='S', help='model architecture')
parser.add_argument('--checkpoint', required=True, type=str, metavar='PATH', help='path to pre-trained checkpoint file')
parser.add_argument('--mode', required=True, type=str, metavar='S', help='tuning/eval mode: lineval or finetune')
...
def main():
    ...
    assert args.mode == 'lineval' or args.mode == 'finetune'
    if args.save == '':
        args.save = f'./outputs/seed_{args.seed}/moco/{args.arch}/{args.mode}/{args.num_train_folds}_folds/{args.label_type}'
    ...
    flog = open(os.path.join(args.save, 'eval_log.txt'), 'w')
    ...
    if args.mode == 'lineval':
        train_transform = transforms.Compose([
            ecg_transforms.AddGaussianNoise(mean=0.0, std=0.01),            # Add mild Gaussian noise
    ...
    # remove BN layer in classification head
    model[-1][2] = nn.Identity()
    # remove Dropout layer in classification head
    model[-1][3] = nn.Identity()
    print(model)
    ...
    if args.checkpoint:
        if os.path.isfile(args.checkpoint):
            ...
            state_dict = {'.'.join(k.split('.')[2:]): v for k, v in checkpoint["state_dict"].items() if k.startswith('encoder_q.features')}
            model.load_state_dict(state_dict, strict=False)
            print("=> loaded pre-trained checkpoint '{}'".format(args.checkpoint))
        else:
            print("=> no checkpoint found at '{}'".format(args.checkpoint))
    else:
        raise AttributeError("=> no pre-trained checkpoint given")
    ...
    for name, param in model.named_parameters():
        if name.startswith('8'):
            ...
            param_head.append(param)
        else:
            # encoder params
            param_encoder.append(param)
    
    if args.mode == 'lineval':
        for param in param_encoder:
            param.requires_grad = False  # not update by gradient
        optimizer = torch.optim.AdamW(param_head, lr=1e-2, weight_decay=1e-6)
    elif args.mode == 'finetune':
        # Fine-tuning the encoder and linear layer with different learning rates
        optimizer = torch.optim.AdamW([
            {'params': param_encoder, 'lr': 1e-4, 'weight_decay': 1e-2},
            {'params': param_head, 'lr': 1e-3, 'weight_decay': 1e-2}
        ])
        pass
    else:
        raise ValueError
